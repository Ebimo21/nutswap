    ...
    return(
      <div onClick={stopFn} className={` ${cardOpen ? "animate-refine-slide-in": "" } an ani z-20 text-white fixed bg-black rounded-tl-xl rounded-tr-xl py-6 px-6 left-0  bottom-0 `}>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-x-2 gap-y-4">
            <SlippageBadge fn={handleSetSlippage} style={` ${slippage == 1 ? "border-[rgba(255,255,108,0.5)] border text-[rgba(255,255,108,0.5)] ": "border-2 border-[#1b1b1a]"} w-24 text-sm py-2 bg-transparent `} amount={1} />
            <SlippageBadge fn={handleSetSlippage} id="five" style={` ${slippage == 5 ? "border-[rgba(255,255,108,0.5)] border text-[rgba(255,255,108,0.5)]": "border-2 border-[#1b1b1a]"} w-24 text-sm py-2 bg-transparent`} amount={5} />
